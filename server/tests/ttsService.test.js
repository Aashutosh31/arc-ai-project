...
check("server TTS is off by default with no provider key", () => {
  const prevProvider = process.env.TTS_PROVIDER;
  const prevGemini = process.env.GEMINI_API_KEY;
  const prevSarvam = process.env.SARVAM_API_KEY;
  ...
  if (prevSarvam !== undefined) process.env.SARVAM_API_KEY = prevSarvam;
});
check("sarvam fallback activates explicitly with SARVAM_API_KEY", () => {
  const prevProvider = process.env.TTS_PROVIDER;
  const prevVoiceProvider = process.env.VOICE_TTS_PROVIDER;
  const prevSarvam = process.env.SARVAM_API_KEY;
  delete process.env.TTS_PROVIDER;
  process.env.VOICE_TTS_PROVIDER = "sarvam";
  process.env.SARVAM_API_KEY = "dummy";
  assert.strictEqual(ttsService.isServerTtsActive(), true);
  assert.strictEqual(ttsService.getDefaultTtsLanguage(), "en-IN");
  if (prevProvider !== undefined) process.env.TTS_PROVIDER = prevProvider;
  else delete process.env.TTS_PROVIDER;
  if (prevVoiceProvider !== undefined)
    process.env.VOICE_TTS_PROVIDER = prevVoiceProvider;
...
check("splits sentences at natural boundaries", () => {
  const segments = ttsService.splitIntoSpeechSegments(
...
check("strips code blocks, links and markdown", () => {
  const segments = ttsService.splitIntoSpeechSegments(
    "Run `npm test` now. See [docs](https://example.com/x) for details. ```code();``` Done.",
  );
...
check("handles Unicode and empty input", () => {
  ...
  assert.deepStrictEqual(ttsService.splitIntoSpeechSegments("   "), []);
  const segments = ttsService.splitIntoSpeechSegments(
    "Emoji \u{1F600} test. Lone \uD800 surrogate here.",
  );
...
check("produces a valid WAV header", () => {
  ...
  const wavBase64 = ttsService.pcm16ToWavBase64(pcm, {
    sampleRate: 24000,
    channels: 1,
  });
...
checkAsync("emits audio segments in order then isFinal", async () => {
  ...
  await buffer.flush();
  const audioEvents = socket.emits.filter(
    (e) => e.event === "ai:tts:audio" && !e.data.isFinal,
  );
  ...
  const finals = socket.emits.filter(
    (e) => e.event === "ai:tts:audio" && e.data.isFinal,
  );
...
checkAsync("stop() halts synthesis and emits audio:stop", async () => {
  ...
  buffer.push(
    "A fairly long first sentence for the test. Another sentence here.",
  );
  buffer.stop();
  await buffer.flush();
...
console.log("\n5. Safe errors");
checkAsync(
  "synthesize without key throws a configured error (no live call)",
  async () => {
...
console.log("\n10. Deterministic ARC-AI pronunciation");
const spokenCases = [
  ["ARC-AI", "A R C A I"],
  ["ARC AI", "A R C A I"],
  ["Arc-AI", "A R C A I"],
  ["arc-ai", "A R C A I"],
  ["ARC A.I.", "A R C A I"],
  ["uses ARC-AI for analysis", "uses A R C A I for analysis"],
  ["The ARC-AI model helps.", "The A R C A I model helps."],
];
for (const [input, want] of spokenCases) {
  check(
    `cleanTextForSpeech pronounces ${JSON.stringify(input)} as ${JSON.stringify(want)}`,
    () => {
      assert.strictEqual(ttsService.cleanTextForSpeech(input), want);
    },
  );
}
check(
  "cleanTextForSpeech maps ARC-AI to Devanagari letters for Hindi TTS",
  () => {
    assert.strictEqual(
      ttsService.cleanTextForSpeech("ARC-AI", "hi-IN"),
      "ए आर सी ए आई",
    );
  },
);
check("cleanTextForSpeech never touches unrelated text", () => {
  assert.strictEqual(
    ttsService.cleanTextForSpeech("The arc industry rallied"),
    "The arc industry rallied",
  );
  assert.strictEqual(
    ttsService.cleanTextForSpeech("ARCs are plural"),
    "ARCs are plural",
  );
  assert.strictEqual(
    ttsService.cleanTextForSpeech("AI tools help"),
    "AI tools help",
  );
});
