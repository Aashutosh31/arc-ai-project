...
const {
  buildMemoryContext,
  searchWorkspace,
} = require("./workspaceSearchService");
...
// This is the single, deliberately cheap gate in front of MCP discovery.
// Discovery may reconnect servers and materialize a large schema inventory, so
// doing it for ordinary conversation puts it directly on the first-token
// critical path.  Capability verbs alone are not enough ("show me React" is
...
const CONTEXT_BUDGET_USER_MESSAGE =
  "That request exceeds the current AI provider's context budget. Please start a new conversation or shorten the request and try again.";
...
const hasDateTimeSignal = (text) => {
  ...
  const hasTimePattern =
    /\b\d{1,2}(:\d{2})?\s?(am|pm)\b|\bat\s+\d{1,2}(:\d{2})?\b/i.test(text);
  const hasRelativePattern =
    /\bin\s+\d+\s+(minute|minutes|hour|hours|day|days|week|weeks)\b/i.test(
      text,
    );
...
const parseDurationMinutes = (text) => {
  const match = String(text || "").match(
    /\bfor\s+(\d+)\s*(minute|minutes|hour|hours)\b/i,
  );
  if (!match) return 30;
...
const parseMeetingTitle = (text) => {
  ...
  const calledMatch = raw.match(
    /\b(?:called|titled)\s+(.+?)(?:\s+for\s+\d+\s*(?:minute|minutes|hour|hours)\b|$)/i,
  );
  if (calledMatch && calledMatch[1]) return calledMatch[1].trim();

  const aboutMatch = raw.match(
    /\b(?:meeting|call|event)\s+(?:about|for)\s+(.+?)(?:\s+at\b|\s+tomorrow\b|\s+today\b|\s+on\b|\s+for\s+\d+\s*(?:minute|minutes|hour|hours)\b|$)/i,
  );
...
class AIService {
  ...
  resolvePendingSchema(toolName, offeredTools, mcpSchemas) {
    ...
    for (const pool of pools) {
      const hit = (Array.isArray(pool) ? pool : []).find(
        (s) => s?.function?.name === toolName,
      );
      if (hit) return hit;
    }
    return null;
  ...
  mcpInventoryBlockForTools(
    finalTools,
    { metadata = null, failures = [] } = {},
  ) {
    ...
    try {
      ...
      if (!mcpTools.length) {
        ...
        let inv = null;
        try {
          inv = buildMcpCapabilityInventory(mcpTools, {
            metadata,
            failures: [],
          });
        } catch {
      ...
      try {
        ...
        failureLines = fails
          .map((f) => {
            ...
            const reason = String(f?.reason || "connection failed").slice(
              0,
              120,
            );
  ...
  buildMcpPlanSummary({
    ...
    try {
      ...
      const required = (
        Array.isArray(requiredNames) ? requiredNames : []
      ).filter((n) => typeof n === "string");
      const selected = (Array.isArray(selectedTools) ? selectedTools : [])
        .map((s) => s?.function?.name)
        .filter((n) => typeof n === "string" && McpToolSource.isMcpToolName(n));
      ...
      const executable = suppressed ? false : required.length > 0 || !needsMcp;
      return {
        intent,
  ...
  mcpDegradedNotice({
    ...
    try {
      ...
      const picked = Array.isArray(capabilityNames)
        ? capabilityNames.filter((n) => typeof n === "string")
        : [];
  ...
  logMcpCapabilityMiss({
    ...
    try {
      ...
      const pool =
        Array.isArray(scopedSchemas) && scopedSchemas.length
          ? scopedSchemas
          : Array.isArray(mcpSchemas)
            ? mcpSchemas
            : [];
      const byName = new Map(
        pool
          .map((s) => [s?.function?.name, s])
          .filter(([n]) => typeof n === "string"),
      );
      const picked = new Set(
        (Array.isArray(capabilityNames) ? capabilityNames : []).filter(
          (n) => typeof n === "string",
        ),
      );
      const stem = (t) => {
        const s = String(t || "").toLowerCase();
        return s.length > 4 && s.endsWith("s") && !s.endsWith("ss")
          ? s.slice(0, -1)
          : s;
      };
      ...
      for (const cap of wanted) {
        ...
        for (const n of picked) {
          try {
            if (declareToolCapabilities(byName.get(n))?.has(cap)) {
              covered = true;
              break;
            }
          } catch {
        ...
        for (const s of pool) {
          ...
          let te = "";
          try {
            te = String(toolEntityStem(s) || "").toLowerCase();
          } catch {
            te = "";
          }
          if (!te) continue;
          ...
          for (const q of queryStems) {
            if (q === teStem || `${q}s` === teStem || q === `${teStem}s`) {
              match = true;
              break;
            }
          }
          ...
          let declared = [];
          try {
            declared = [...(declareToolCapabilities(s) || [])];
          } catch {
            declared = [];
          }
          let key = "";
          try {
            key = mcpServerKeyOf(s);
          } catch {
            key = "";
          }
      ...
      if (misses.length) {
        try {
          console.error("[AIService] mcpCapability.miss", {
            ...
            serverScope: Array.isArray(serverScope)
              ? serverScope.slice(0, 8)
              : [],
          });
        } catch {
          /* diagnostics only */
      ...
      /* diagnostics must never break the path */
    }
    return misses;
  }
    ...
    try {
      ...
      for (const s of exposed) {
        ...
        try {
          ...
          sid =
            m && (m.serverId || m.slug) ? String(m.serverId || m.slug) : null;
        } catch {
          sid = null;
        }
        if (!sid) continue;
      ...
      if (exposed.length === 0) {
        try {
          const total =
            typeof registry.count === "number" ? registry.count : null;
          if (typeof total === "number" && total > 0)
            gaps.push({ serverId: "*", exposed: 0, registered: total });
        } catch {
          /* conservative: skip */
        }
      }
  ...
  missingToolRetrySpec(err, tools, mcpSchemas, protectedNames) {
    try {
      ...
      const shielded = new Set(
        (Array.isArray(protectedNames) ? protectedNames : []).filter(
          (n) => typeof n === "string",
        ),
      );
      shielded.add(name);
      ...
      while (rest.length + 1 > 6) {
        ...
        for (let i = rest.length - 1; i >= 0; i -= 1) {
          if (!shielded.has(rest[i]?.function?.name)) {
            victim = i;
            break;
          }
        }
  ...
  looksLikeClarification(prose) {
    try {
      ...
      return /\b(which|what|where|when|who|whom|how\s+(many|much)|let me know|could you|can you\s+(tell|share|provide|specify|confirm)|i need|need to know|need more|missing|specify|specifies|clarify|tell me)\b/i.test(
        text,
      );
    } catch {
      return false;
  ...
  planMcpRecovery({
    ...
    try {
      ...
      const required = (
        Array.isArray(requiredSchemas) ? requiredSchemas : []
      ).filter((s) => s && typeof s?.function?.name === "string");
      if (!required.length) return null;
      if (!proseBad && !emptyAnswer && this.looksLikeClarification(proseText))
        return null;
      const retryTools = required.slice(0, 6);
      if (!retryTools.length) return null;
      const forcedTool =
        retryTools.length === 1 ? retryTools[0].function.name : null;
  ...
  hasEntityIdValue(schema, args) {
    try {
      ...
      for (const [name, value] of Object.entries(bag)) {
        ...
        let kind = "";
        try {
          kind = this.classifyMissingParam(name, entity).kind;
        } catch {
          kind = "";
        }
  ...
  upsertUpdateMode(schema, { intentCaps = null, text = "", args = null } = {}) {
    try {
      ...
      let pasted;
      try {
        pasted = extractIdentifierValue(text, "id");
      } catch {
        pasted = undefined;
      }
  ...
  resolveEnforcementArgs(schema, text, { updateMode = false } = {}) {
    try {
      ...
      const isOpenString = (p) => {
        ...
        return (
          !isIdentifierParam(p.name) &&
          !(Array.isArray(d.enum) && d.enum.length) &&
          !["boolean", "bool", "integer", "number"].includes(
            String(d.type || "string").toLowerCase(),
          )
        );
      };
      ...
      const defaultedNames = Object.keys(extracted).filter((k) => {
        const f = found[k];
        return (
          f === undefined || f === null || (typeof f === "string" && !f.trim())
        );
      });
      let check = validateArgs(schema, extracted);
      if (check.ok !== true && quotes.length) {
        const unresolvedOpens = missingRequired(schema, extracted).filter((n) =>
          opens.some((p) => p.name === n),
        );
        if (unresolvedOpens.length === 1) {
          check = validateArgs(schema, {
            ...extracted,
            [unresolvedOpens[0]]: quotes[0],
          });
        }
      }
      const verify = (coerced, base) => {
        const valueOf = (name) =>
          coerced && coerced[name] !== undefined ? coerced[name] : base[name];
      ...
      if (opens.length === 1 && quotes.length) {
        const trial = validateArgs(schema, {
          ...extracted,
          [opens[0].name]: quotes[0],
        });
  ...
  async runMcpRecovery({
    ...
    const scope = Array.isArray(serverScope)
      ? serverScope.filter((s) => typeof s === "string" && s)
      : [];
    try {
      if (!plan || !Array.isArray(plan.retryTools) || !plan.retryTools.length)
        return fail();
      ...
      try {
        retryIntent = classifyIntentCapabilities(baseText);
      } catch {
        retryIntent = new Set();
      }
      const retryTools = [];
      ...
      try {
        for (const schema of plan.retryTools || []) {
          const check = this.gatePreflightCandidate(
            schema,
            retryIntent,
            baseText,
          );
          if (check.ok) {
            ...
            try {
              console.error("[AIService] mcpRecovery.irrelevantRejected", {
                tool: check.name,
                reason: check.reason,
              });
            } catch {
            ...
            gatedOut.push({
              tool: check.name || schema?.function?.name,
              missing: [],
              options: {},
        ...
        for (const s of plan.retryTools || [])
          if (!retryTools.includes(s)) retryTools.push(s);
      }
      if (!retryTools.length) {
        return fail({
          capabilityMismatch: gatedOut.length > 0,
          unresolved: gatedOut,
        });
      }
      const retryNames = new Set(
        retryTools.map((s) => s?.function?.name).filter(Boolean),
      );
      const forcedName =
        typeof plan.forcedTool === "string" && retryNames.has(plan.forcedTool)
          ? plan.forcedTool
          : null;
      const retryPrompt = this.refreshInventoryPrompt(
        systemPrompt,
        this.mcpInventoryBlockForTools(retryTools, {
      ...
      const gen =
        typeof generate === "function"
          ? generate
          : (req) => this.llmRouter.generate(req);
      ...
      try {
        exposure = partitionToolCallsByExposure(calls, retryTools);
      } catch {
        /* fail open */
      }
      if (exposure.unexposedNames.length) {
        console.error("[AIService] recovery tool-call exposure violation", {
        ...
        try {
          for (const schema of retryTools) {
            ...
            if (scope.length) {
              let key = "";
              try {
                key = mcpServerKeyOf(name);
              } catch {
                key = "";
              }
              let inScope = false;
              try {
                inScope = scope.some((s) => mcpScopeMatches(s, key));
              } catch {
                inScope = false;
              }
              if (!key || !inScope) {
                try {
                  console.error("[AIService] mcpRecovery.scopeRejected", {
                    tool: name,
                    scope,
                  });
                } catch {
            ...
            let resolved = null;
            const updateMode = this.upsertUpdateMode(schema, {
              intentCaps: retryIntent,
              text: baseText,
            });
            try {
              resolved = this.resolveEnforcementArgs(schema, baseText, {
                updateMode,
              });
            } catch {
              resolved = null;
            ...
            if (!resolved || !resolved.ok) {
              if (!unresolved.some((u) => u.tool === name)) {
                ...
                try {
                  ...
                  missing = [
                    ...new Set([...v.errors.map((e) => e.name), ...extra]),
                  ];
                } catch {
                  missing = [];
                }
                if (missing.length)
                  unresolved.push({ tool: name, missing, options: {} });
              }
              continue;
            }
            forced.push({
          ...
          /* deterministic construction must never throw */
        }
        const mutations = retryTools.filter(isMutationSchema);
        const forcedNames = new Set(forced.map((c) => c?.function?.name));
        const allMutationsResolved = mutations.every((s) =>
          forcedNames.has(s?.function?.name),
        );
      ...
      try {
        const executedNames = deduped
          .map((t) => t?.function?.name || t?.name)
          .filter(Boolean);
        console.log("[AIService] mcpRecovery.executed", {
          tools: executedNames,
        });
      } catch {
        /* diagnostics only */
      ...
      console.log("[AIService] mcpRecovery.failed", {
  ...
  gatePreflightCandidate(schema, intentCaps, baseText) {
    try {
      ...
      let declared = null;
      try {
        declared = declareToolCapabilities(schema);
      } catch {
        declared = null;
      }
      if (!declared)
        return { ok: false, reason: "no-declared-capability", name };
      const sharesIntent =
        intent.size === 0
          ? true
          : [...declared].some(
        ...
        if (wantsStrictMutation) {
          ...
          if (te) {
            ...
            const entityOk = [...queryStems].some((q) => {
              const qs = stem(q);
              return (
                qs === teStem || `${qs}s` === teStem || qs === `${teStem}s`
              );
            });
            if (!entityOk)
              return { ok: false, reason: "entity-mismatch", name };
          }
        }
      } catch {
        /* entity evidence is advisory */
      }
      return { ok: true, reason: null, name };
    } catch {
      return {
        ok: false,
        reason: "no-declared-capability",
    ...
    try {
      ...
      for (const schema of required) {
        ...
        if (!offeredNames.has(name)) {
          try {
            console.error("[AIService] mcpPreflight.notCallable", {
              tool: name,
            });
          } catch {
            /* diagnostics only */
        ...
        if (scope.length) {
          ...
          if (!key || !inScope) {
            try {
              console.error("[AIService] mcpPreflight.scopeRejected", {
                tool: name,
                scope,
              });
            } catch {
      ...
      try {
        for (const schema of scoped) {
          ...
          if (!check.ok) {
            try {
              console.error("[AIService] mcpPreflight.irrelevantRejected", {
                tool: name,
                reason: check.reason,
              });
            } catch {
        ...
        for (const s of scoped) if (!gated.includes(s)) gated.push(s);
      }
      if (!gated.length) {
        try {
      ...
      try {
        ...
        for (const schema of gated) {
          ...
          const cap =
            (Array.isArray(CAPABILITY_PICK_ORDER)
          ...
          let entity = "";
          try {
            entity = String(toolEntityStem(schema) || "").toUpperCase();
          } catch {
            entity = "";
          }
      ...
      const pool =
        Array.isArray(mcpSchemas) && mcpSchemas.length
          ? mcpSchemas
          : Array.isArray(tools)
            ? tools
            : [];
      ...
      for (const schema of planSchemas) {
        ...
        let r = null;
        try {
          r = this.resolveEnforcementArgs(schema, baseText, { updateMode });
        } catch {
          r = null;
        }
        if (r && r.ok) {
          ...
          if (updateMode && mutationNames.has(name)) {
            let hasId = false;
            try {
              hasId = this.hasEntityIdValue(schema, r.args || {});
            } catch {
              hasId = false;
            }
            if (!hasId) continue;
        ...
        if (
          ...
          try {
            ...
            if (live.ok) {
              try {
                console.log("[AIService] mcpPreflight.contextResolved", {
                  tool: name,
                });
              } catch {
                /* diagnostics only */
      ...
      const emit = (event, payload) => {
        try {
          ...
          /* status must never break execution */
        }
      };
      const planTitle =
        String(title || baseText || "").slice(0, 120) || "MCP execution";
      const stepIds = resolved.map((_, i) => `${executionId}-step-${i}`);
      emit("execution.created", {
        executionId,
        title: planTitle,
        steps: resolved.map((r, i) => ({
          id: stepIds[i],
          tool: r.name,
          status: "PLANNED",
        })),
      ...
      for (let i = 0; i < resolved.length; i += 1) {
        ...
        if (
          ...
          socket &&
          !socket.isInterrupted
        ) {
          try {
            socket.emit("ai:client:action", content.clientAction);
          } catch {
            /* advisory */
          }
        }
      ...
      console.log("[AIService] mcpPreflight.failed", {
        reason: err?.message || "error",
      });
      return done(emptyPlan(false, false), [], []);
    }
  ...
  isPendingPermitted(toolName, mcpSchemas) {
    ...
    return (Array.isArray(mcpSchemas) ? mcpSchemas : []).some(
      (s) => s?.function?.name === toolName,
    );
  }
  ...
  classifyMissingParam(paramName, toolEntity) {
    try {
      ...
      let stem = raw;
      const snake = stem.match(
        /^(.*)_(id|uuid|url|uri|urn|guid|handle|slug|key)$/i,
      );
      if (snake && snake[1]) {
        stem = snake[1];
      } else {
        const camel = stem.match(
          /^(.*[a-z])(Id|Uuid|Url|Uri|Urn|Guid|Handle|Slug|Key)$/,
        );
      ...
      if (
        entity &&
        (stem === entity || `${stem}s` === entity || stem === `${entity}s`)
      ) {
        return { kind: "entityId", stem };
      }
      if (stem && CREATION_CONTEXT_NOUNS.has(stem))
        return { kind: "context", stem };
  ...
  findContextResolvers(entityStem, mcpSchemas, excludeName = null) {
    ...
    try {
      ...
      if (!stem) return out;
      for (const s of (Array.isArray(mcpSchemas) ? mcpSchemas : []).slice(
        0,
        64,
      )) {
        const name = s?.function?.name;
        ...
        let caps = null;
        try {
          caps = declareToolCapabilities(s);
        } catch {
          caps = null;
        }
        if (
          !caps ||
          (!caps.has("LIST") && !caps.has("READ") && !caps.has("SEARCH"))
        )
          continue;
        let bare = "";
        try {
        ...
        const hit = tokens.some(
          (t) =>
            ...
            (t.length > 4 &&
              stem.length > 4 &&
              (t.startsWith(stem) || stem.startsWith(t))),
        );
        if (!hit) continue;
        out.push({
          schema: s,
          name,
          listFirst: caps.has("LIST") ? 0 : 1,
          capCount: caps.size,
        });
      ...
      return arr
        ...
        .map((it) => {
          if (it && typeof it === "object") {
            const id =
              it.id ?? it.uuid ?? it.key ?? it.identifier ?? it.value ?? null;
  ...
  mapNamedQuotes(text, candidates) {
    ...
    try {
      ...
      for (const m of flat.matchAll(/"([^"]+)"|'([^']+)'/g)) {
        ...
        if (q && flat.includes(q) && q.length < flat.length)
          quotes.push({ text: q, index: m.index });
      }
      ...
      (Array.isArray(candidates) ? candidates : []).forEach((name, ci) => {
        if (typeof name !== "string" || !name || mapping[name] !== undefined)
          return;
        ...
        if (!q) return;
        const qi = quotes.findIndex(
          (qq, idx) => !usedIndices.has(idx) && qq.text === q,
        );
        if (qi === -1) return;
  ...
  async resolveMutationContextLive({
    argSchema,
    execName,
    baseArgs = {},
    userText = "",
    mcpSchemas = [],
    execFn = null,
    budget = CREATION_RESOLVER_BUDGET,
    updateMode = null,
  } = {}) {
    const empty = {
      ok: false,
    ...
    try {
      ...
      try {
        ...
        if (callerKey) {
          const same = pool.filter((s) => {
            try {
              return mcpServerKeyOf(s) === callerKey;
            } catch {
              return false;
            }
          });
        ...
        /* fail open to the supplied pool */
      }
      ...
      if (typeof mode !== "boolean") {
        try {
          mode = extractIdentifierValue(userText, "id") !== undefined;
        } catch {
          mode = false;
        }
      }
      ...
      let merged = {};
      try {
        merged = applyParamDefaults(
          argSchema,
          extractArgValues(userText, params),
        );
      } catch {
        merged = {};
      }
      ...
      const missingNow = () =>
        missingEffective(argSchema, merged, { updateMode: mode });
      ...
      const isContextName = (n) => {
        try {
          return this.classifyMissingParam(n, entity).kind === "context";
        } catch {
          return false;
        }
      };
      ...
      const schemaProps = (() => {
        try {
          return argSchema?.function?.parameters?.properties || {};
        } catch {
          return {};
        }
      })();
      ...
      const named = this.mapNamedQuotes(
        userText,
        [...byName.keys(), ...optionalContent].filter((n) => {
          const d = byName.get(n)?.def;
          if (d !== undefined)
            return isOpenContent({ name: n, def: d }) && !isContextName(n);
      ...
      const needy = params
        .filter(
          (p) =>
            isOpenContent(p) &&
            !isContextName(p.name) &&
            missingNow().includes(p.name),
        )
        .map((p) => p.name);
      ...
      const recheck = () => {
        ...
        let extra = [];
        try {
          extra = missingEffective(argSchema, (v && v.coerced) || merged, {
            updateMode: mode,
          });
        } catch {
          extra = [];
      ...
      for (const { name: pname } of params) {
        ...
        let pasted;
        try {
          pasted = extractIdentifierValue(userText, pname);
        } catch {
          pasted = undefined;
        }
        if (pasted !== undefined) {
        ...
        let result = null;
        try {
          result = await execFn(resolvers[0]?.function?.name, {});
        } catch {
          result = null;
        }
        used += 1;
        ...
        if (items.length === 1 && items[0]) {
          const one = items[0];
          const v = isIdentifierParam(pname)
            ? (one.id ?? one.name)
            : (one.name ?? one.id);
          if (v) merged[pname] = v;
  ...
  async tryCreationContext({
    ...
    try {
      ...
      const pureCreate =
        intentCaps &&
        intentCaps.has("CREATE") &&
        !intentCaps.has("UPDATE") &&
        !intentCaps.has("DELETE");
      if (!pureCreate) return null;
      let baseArgs = {};
      try {
        baseArgs =
          typeof originalCall?.function?.arguments === "string"
  ...
  async matchPendingCandidateIds(
    text,
    pendingState,
    schema,
    mcpSchemas,
    ...
    const fills = {};
    try {
      const missing = Array.isArray(pendingState?.missing)
        ? pendingState.missing
        : [];
      if (!missing.length || typeof execFn !== "function") return fills;
      let pool = Array.isArray(mcpSchemas) ? mcpSchemas : [];
      try {
        const ownerKey = mcpServerKeyOf({
          function: { name: pendingState?.toolName },
        });
        if (ownerKey) {
          const same = pool.filter((s) => {
            try {
              return mcpServerKeyOf(s) === ownerKey;
            } catch {
              return false;
            }
          });
      ...
      for (const name of missing) {
        ...
        if ((cls.kind !== "context" && cls.kind !== "entityId") || !cls.stem)
          continue;
        const resolvers = this.findContextResolvers(
          cls.stem,
          pool,
          pendingState?.toolName,
        );
        if (!resolvers.length) continue;
        let result = null;
        try {
        ...
        if (items.length === 1 && items[0]) {
          const one = items[0];
          const v = isIdentifierParam(name)
            ? (one.id ?? one.name)
            : (one.name ?? one.id);
          if (v) fills[name] = v;
        } else if (items.length > 1) {
  ...
  async tryTargetResolution({
    ...
    try {
      ...
      if (!missed.length || !missed.every((n) => isIdentifierParam(n)))
        return null;
      ...
      if (intentCaps && !intentCaps.has("UPDATE") && !intentCaps.has("DELETE"))
        return null;
      ...
      const resolverNames = resolvers
        .map((s) => s?.function?.name)
        .filter(Boolean);
      console.log("[AIService] targetResolution.start", {
        tool: execName,
        missing: missed,
      ...
      const round1Prompt = this.refreshInventoryPrompt(
        systemPrompt,
        this.mcpInventoryBlockForTools(round1Tools, {}),
      );
      ...
      const round1Calls = Array.isArray(round1?.toolCalls)
        ? round1.toolCalls
        : [];
      // Immediate retry with complete args — no second round needed.
      const directRetry = round1Calls.find(
        (c) => (c?.function?.name || c?.name) === execName,
      );
      if (directRetry) {
        let parsed = {};
        try {
          parsed =
            typeof directRetry.function.arguments === "string"
              ? JSON.parse(directRetry.function.arguments)
              : { ...(directRetry.function.arguments || {}) };
        } catch {
          parsed = {};
        }
        const check = validateArgs(argSchema, { ...originalArgs, ...parsed });
        if (check.ok) {
          console.log("[AIService] targetResolution.direct-retry", {
            tool: execName,
          });
      ...
      const resolverCalls = round1Calls
        .filter((c) => {
      ...
      if (!resolverCalls.length) {
        console.log("[AIService] targetResolution.no-resolver-call", {
          tool: execName,
        });
        return null;
      }
      ...
      for (const tc of resolverCalls) {
        ...
        let result = null;
        try {
          result = await TaskExecutor.executeTool(fname, args, userId, socket, {
            signal,
            conversationId,
            workspaceId,
          });
        ...
        if (result?.clientAction && socket)
          socket.emit("ai:client:action", result.clientAction);
      ...
      const round2Prompt = this.refreshInventoryPrompt(
        systemPrompt,
        this.mcpInventoryBlockForTools([argSchema], {}),
      );
      ...
      const round2Calls = Array.isArray(round2?.toolCalls)
        ? round2.toolCalls
        : [];
      const retry = round2Calls.find(
        (c) => (c?.function?.name || c?.name) === execName,
      );
      if (!retry) {
        console.log("[AIService] targetResolution.no-retry", {
          tool: execName,
        });
        return null;
      }
      ...
      try {
        parsed =
          typeof retry.function.arguments === "string"
            ? JSON.parse(retry.function.arguments)
      ...
      const check = validateArgs(argSchema, { ...originalArgs, ...parsed });
      if (!check.ok) {
        console.log("[AIService] targetResolution.retry-invalid", {
          tool: execName,
          missing: check.errors.map((e) => e.name),
        });
        return null;
      ...
      console.log("[AIService] targetResolution.failed", {
        tool: execName,
        reason: err?.message || "error",
      });
      return null;
  ...
  async saveWorkingState(
    ...
    try {
      ...
      if (typeof summary === "string" && summary)
        update.conversationSummary = summary.slice(0, 1200);
      ...
      for (const doc of docs) {
        ...
        for (const call of calls) {
          ...
          if (["playMedia", "stopMedia"].includes(name)) {
            ...
            if (title)
              state.activeMedia = { tool: name, title: title.slice(0, 140) };
          } else if (
            ["webSearch", "scrapeWebsite", "getTopNews"].includes(name)
          ) {
            const query = compactStr(
              input.query || input.q || input.url || input.topic,
            );
            let refs = [];
            try {
              const results =
                output?.results || output?.articles || output?.items;
              if (Array.isArray(results)) {
                refs = results
                  .slice(0, 3)
                  .map((r) => compactStr(r?.title || r?.url || r, 100))
                  .filter(Boolean);
              }
            } catch {
              refs = [];
            ...
            if (ref)
              state.activeResource = { tool: name, ref: ref.slice(0, 140) };
  ...
  async refreshWorkingStateFromResults(
    conversationId,
    userId,
    executedResults,
    pendingState = null,
    ...
    if (
      isGuestActorId(userId) ||
      !conversationId ||
      !Array.isArray(executedResults) ||
    ...
    try {
      ...
      const base =
        convo && convo.workingState && typeof convo.workingState === "object"
          ? { ...convo.workingState }
          : {};
      const compactStr = (v, cap = 140) => {
        let s = "";
      ...
      for (const entry of executedResults) {
        ...
        if (typeof content === "string") {
          try {
            ...
            content = { message: String(entry.content).slice(0, 300) };
          }
        }
        ...
        if (["playMedia", "stopMedia"].includes(name)) {
          const title = compactStr(
            payload?.clientAction?.title || payload?.message || payload?.title,
          );
          ...
          try {
            const results =
              payload?.results || payload?.articles || payload?.items;
            if (Array.isArray(results))
              refs = results
                .slice(0, 3)
                .map((r) => compactStr(r?.title || r?.url || r, 100))
          ...
          const ref = compactStr(
            payload?.title || payload?.id || payload?.message,
          );
          if (ref) base.activeResource = { tool: name.slice(0, 80), ref };
        } else {
          const ref = compactStr(
            payload?.message || payload?.title || payload?.summary,
          );
  ...
  async executeSchedulingPipeline(userId, rawText, socket, signal = null) {
    ...
    console.log(
      `[Planner] Chosen tools (in order): ${plannedTools.join(" -> ")}`,
    );

    console.log("[Planner] Before tool execution:", {
      tool: "checkCalendar",
      args: checkArgs,
    });
    const checkResult = await TaskExecutor.executeTool(
      "checkCalendar",
      checkArgs,
      userId,
      socket,
    ...
    console.log("[Planner] After tool execution payload:", {
      tool: "checkCalendar",
      payload: checkResult,
    });
    ...
    try {
      ...
      scheduleResult = await TaskExecutor.executeTool(
        "scheduleMeeting",
        scheduleArgs,
        userId,
        socket,
      ...
      console.error(
        "[Planner] scheduleMeeting pipeline error:",
        error?.stack || error,
      );
      return "Meeting request processed but confirmation pending.";
    }
    ...
    if (!scheduleResult?.success) {
      return (
        scheduleResult?.error || "Unable to schedule the meeting right now."
      );
    }
  ...
  async processQuery(
    ...
    try {
      resolvedWorkspace = await this.workspaceRuntime.resolveWorkspace({
        userId,
        workspaceId,
      });
      console.info(
        "[AIService] workspace resolved for incoming request:",
        resolvedWorkspace?._id || null,
        "namespace:",
        resolvedWorkspace?.vectorNamespace || null,
      );
    } catch (err) {
      console.warn(
        "[AIService] workspace resolution failed (pre-conversation):",
        err?.message || err,
      );
    }
    const workspaceContext =
      this.workspaceRuntime.injectWorkspaceContext(resolvedWorkspace);
    ...
    if (!isGuestActorId(userId)) {
      try {
        ...
        // Saving an already-addressed user turn must not delay the response
        // stream.  A newly-created conversation above is still awaited for
        // its id; the message write is durable but deliberately detached from
        ...
        void Message.create({
          ...
          role: "user",
          content:
            text || (document ? `Attached document: ${document.name}` : ""),
          attachments: imageBase64
            ? [{ type: "image" }]
            : document
              ? [{ type: "document", name: document.name }]
              : [],
          metadata: {
            streaming: false,
          },
        }).catch((err) => {
          console.error("[AIService] Error saving user message:", err);
        });
      } catch (err) {
      ...
      onFirstChunk: () => {
        if (deliveryTiming.firstChunkAt == null) {
          ...
          console.log("[AIService] delivery.firstChunk", {
            // T4 marker: first LLM text chunk left the provider.
            at: deliveryTiming.firstChunkAt,
            elapsedMs:
              deliveryTiming.firstChunkAt - deliveryTiming.providerStartAt,
    ...
    let ttsBuffer = null;
    // Voice Runtime 2.0 streamer: same lifetime as ttsBuffer, emits the
    // binary `voice:tts:*` PCM channel alongside the legacy WAV events.
    // Never blocks text delivery; AbortSignal cancels both paths.
    let voiceStreamer = null;
    let assistantDraftMessageId = null;
    let assistantDraftContent = "";
    let finalOutputText = "";
    // True when the INITIAL provider round streamed (voice-first
    // delivery): text reached the socket and TTS while generation was
    ...
    try {
      ...
      const user = isGuest || isFirstConversationTurn
        ? null
        : await User.findById(userId)
            .select(
              "preferences.memoryLearningEnabled preferences.voice preferences.accentColor",
            )
            .lean();
      const memoryLearningEnabled =
        user?.preferences?.memoryLearningEnabled !== false;

      const persistAssistantDraft = async (
        nextContent,
        { interrupted = false, state = "streaming" } = {},
      ) => {
        if (isGuestActorId(userId) || !conversationId) return;
      ...
      if (calendarIntent.type === "schedule" && !imageBase64 && !document) {
        const scheduleResponse = await Promise.race([
          this.executeSchedulingPipeline(
            userId,
            text,
            socket,
            controller.signal,
          ...
          new Promise((resolve) => {
            setTimeout(
              () =>
                resolve("Meeting request processed but confirmation pending."),
              5000,
            );
          }),
        ]);
      ...
      const memoryDocs = isGuest || isFirstConversationTurn
        ? []
        : await AIMemory.find({
            userId,
            workspaceId: workspaceContext.workspaceId || null,
      ...
      let retrievalItems = [];
      if (!isGuest && !isFirstConversationTurn) {
        try {
          const ctx = await WorkspaceContextManager.getActiveContext({
            userId,
            conversationId,
          ...
          console.warn(
            "[AIService] WorkspaceContextManager failed, falling back to buildMemoryContext",
            err?.message || err,
          );
          retrievalItems = await buildMemoryContext({
            userId,
            query: baseMessageContent,
            limit: 10,
            workspaceId,
        ...
        socket.emit("ai:agent:status", {
          ...
          detail:
            retrievalItems.length > 0
              ? `${retrievalItems.length} relevant workspace items prepared.`
              : "No strong memory matches found.",
        });
      }
      ...
      if (document) {
        try {
          console.log(
            `[Agent Router] Reading attached document: ${document.name}`,
          );
          let parsedText = "";
          ...
          if (document.type === "application/pdf") {
            ...
            parsedText = Buffer.from(document.base64, "base64").toString(
              "utf-8",
            );
          }
          ...
          if (parsedText.length > DOC_CHARS_INITIAL) {
            parsedText =
              parsedText.substring(0, DOC_CHARS_INITIAL) +
          ...
          console.error(
            "[Agent Router] Document parse error:",
            err.message || err,
          );
      ...
      try {
        ...
        persistedWorkingState =
          convoState &&
          convoState.workingState &&
          typeof convoState.workingState === "object"
            ? convoState.workingState
            : null;
        persistedSummary = String(convoState?.conversationSummary || "");
      } catch {
        persistedWorkingState = null;
        persistedSummary = "";
      }
      ...
      const toolSelectionQuery = this.buildToolSelectionQuery(
        baseMessageContent,
        recentTurns,
      );
      ...
      {
        let pendingState = await this.loadPendingToolCall(
          conversationId,
          userId,
        );
        const failClosed = async (reason, extra = {}) => {
          console.log("[AIService] pendingTool.abandoned", {
            reason,
            ...extra,
          });
          await this.clearPendingToolCall(conversationId, userId);
        ...
        if (pendingState) {
          ...
          if (!this.isPendingPermitted(pTool, mcpSchemas)) {
            ...
            Array.isArray(toolPick.mcpExplicit) &&
            toolPick.mcpExplicit.length > 0 &&
            !toolPick.mcpExplicit.includes(pTool)
          ) {
            // User pivoted to a different explicit tool — replace.
            console.log("[AIService] pendingTool.replaced", {
              from: pTool,
              to: toolPick.mcpExplicit[0],
            });
            await failClosed("replaced", {
              from: pTool,
              to: toolPick.mcpExplicit[0],
            });
          } else {
            const schema = this.resolvePendingSchema(
              pTool,
              toolPick.tools,
              mcpSchemas,
            );
            if (!schema) {
              ...
              try {
                ...
                /* candidate matching must never break pending */
              }
              ...
              if (advanced.action === "execute") {
                console.log("[AIService] pendingTool.execute", {
                  tool: pTool,
                  missing: [],
                });
                await this.clearPendingToolCall(conversationId, userId);
                ...
                console.log("[AIService] pendingTool.ask", {
                  tool: pTool,
                  missing: advanced.missing,
                });
                await this.savePendingToolCall(
                  conversationId,
                  userId,
                  advanced.pending,
                );
                ...
                await failClosed(advanced.reason || "abandoned", {
                  tool: pTool,
                });
              }
            }
        ...
        if (
          ...
          const explicitUpdateMode = this.upsertUpdateMode(schema, {
            intentCaps: toolIntentCaps,
            text: baseMessageContent,
          });
          const explicitParams = effectiveRequiredParams(schema, {
            updateMode: explicitUpdateMode,
          });
          if (
            schema &&
            explicitParams.length > 0 &&
            this.isPendingPermitted(target, mcpSchemas)
          ) {
            ...
            const explicitCheck = (() => {
              try {
                ...
                const extra = missingEffective(
                  schema,
                  (v && v.coerced) || extracted,
                  { updateMode: explicitUpdateMode },
                );
                ...
                return validateArgs(schema, extracted);
              }
            })();
            ...
            if (!check.ok) {
              ...
              console.log("[AIService] pendingTool.created", {
                tool: target,
                missing: created.missing,
              });
              await this.savePendingToolCall(conversationId, userId, created);
            } else {
              console.log("[AIService] pendingTool.execute-immediate", {
                tool: target,
                missing: [],
              });
              syntheticResponse = {
        ...
        if (pendingForState || persistedWorkingState) {
          turnWorkingState = {
            ...
            ...(pendingForState && typeof pendingForState.toolName === "string"
              ? {
                  pendingTool: {
                    tool: pendingForState.toolName,
                    missing: pendingForState.missing || [],
                  },
                }
              : {}),
          };
      ...
      const activeStateTool =
        turnWorkingState?.activeMedia?.tool ||
      ...
      try {
        const finalBlock = this.mcpInventoryBlockForTools(tools, {
          metadata: mcpMetadata,
          failures: mcpFailures,
        });
        systemPrompt = this.refreshInventoryPrompt(systemPrompt, finalBlock);
      } catch {
        /* inventory refresh must never break the path */
      }
      ...
      // T3 marker: the LLM request started (voice path begins here after
      // ai:stt:final). Delta to the first delivery.firstChunk = first-token.
      try {
        console.log(
          "[VoiceLatency] llm.request at=%d",
          deliveryTiming.providerStartAt,
        );
      } catch {
      ...
      if (!syntheticResponse && !skipMcpGate) {
        ...
        if (
          preflightResult &&
          Array.isArray(preflightResult.executed) &&
          preflightResult.executed.length
        ) {
      ...
      // Initial-request streaming (voice-first latency): stream the first
      // provider round so the earliest tokens reach the browser and the
      // server-TTS buffers while the rest of the answer is still being
      // generated. Skipped whenever required MCP capabilities are in play
      // — those turns must keep the buffered round so enforcement can
      // suppress/replace a probe response before it is shown (§7/§8
      // invariance). Non-streamed initial rounds keep the legacy
      // buffered path byte-for-byte.
      const initialStreamAllowed =
        !syntheticResponse &&
        (!Array.isArray(toolPick?.mcpCapability) ||
          toolPick.mcpCapability.length === 0) &&
        (!Array.isArray(supportingMcp) || supportingMcp.length === 0);
      const generateInitial = (useTools, usePrompt) =>
        this.llmRouter.generate({
          messages,
          systemPrompt: usePrompt,
          tools: useTools,
          stream: initialStreamAllowed,
          maxTokens,
      ...
      let toolCalls = [];
      if (
        preflightExecutedById.size > 0 &&
        preflightResult &&
        Array.isArray(preflightResult.toolCalls)
      ) {
        ...
        if (!response) {
          try {
            ...
            const retrySpec = this.missingToolRetrySpec(
              genErr,
              tools,
              mcpSchemas,
              mcpProtectedNames,
            ...
            console.log("[AIService] toolMismatch.retry", {
              tool: retrySpec.name,
            });
            ...
            try {
              systemPrompt = this.refreshInventoryPrompt(
                systemPrompt,
                this.mcpInventoryBlockForTools(tools, {
                  metadata: mcpMetadata,
                  failures: mcpFailures,
                }),
              );
        ...
        console.log("[AIService] provider.completed", {
          elapsedMs:
            deliveryTiming.providerDoneAt - deliveryTiming.providerStartAt,
        ...
        assistantResponseMeta.tokens = response?.tokens || {
          input: 0,
          output: 0,
        };
        ...
        const serverTtsActive =
          Boolean(socket) && ttsService.isServerTtsActive();
        // Per-turn language from STT auto-detection (Sarvam), else the
        // provider default. Carried into both TTS paths so the
        // deterministic pronunciation layer can emit script-appropriate
        // letter spellings (Devanagari for hi-*, Latin otherwise).
        ...
        if (socket) {
          socket.emit("ai:tts:mode", {
            mode: serverTtsActive ? "server" : "browser",
          });
        }
        ...
        // ---- Initial-request streaming (voice-first latency) ----
        // When the initial round streamed, consume it HERE in real time:
        // each provider chunk is delivered to the socket and pushed into
        // the server-TTS buffers as it arrives, so the first natural
        // sentence reaches synthesis while the rest of the answer is
        // still generating. Follow-up tool calls ride the terminal
        // (textless) chunk exactly like continuation turns. Abort/error
        // propagates to the outer try/catch (TTS stop + partial
        // persistence run there unchanged).
        initialStreamed = Boolean(
          initialStreamAllowed &&
          response &&
          typeof response.stream !== "undefined",
        );
        if (initialStreamed) {
          const initialStreamOnChunk =
            ttsBuffer || voiceStreamer
              ? async (chunkText) => {
                  assistantDraftContent += chunkText;
                  if (ttsBuffer) ttsBuffer.push(chunkText);
                  if (voiceStreamer) voiceStreamer.push(chunkText);
                  // Persistence is deliberately after first-audio scheduling:
                  // a database round trip must not sit in front of TTS for
                  // every streamed LLM delta.
                  await persistAssistantDraft(assistantDraftContent, {
                    interrupted: false,
                    state: "streaming",
                  });
          ...
          const tapped = (async function* initialTap() {
            for await (const chunk of response.stream) {
              if (
                ...
                initialFollowUp.push(...chunk.toolCalls);
              }
              yield chunk;
            }
          })();
          finalOutputText =
            (await this.llmRouter.streamingRuntime.consume(
              tapped,
              socket,
              controller.signal,
              initialStreamOnChunk || null,
              deliveryHooks,
            )) || "";
          if (initialFollowUp.length) toolCalls = initialFollowUp;
          console.log("[AIService] initial.stream.completed", {
            elapsedMs:
              deliveryTiming.providerStartAt != null
                ? Date.now() - deliveryTiming.providerStartAt
                : null,
            followUpToolCalls: Array.isArray(initialFollowUp)
              ? initialFollowUp.length
              : 0,
            outputLength: String(finalOutputText || "").length,
          });
        }
        ...
        if (
          ...
          try {
            ...
            if (exposure.unexposedNames.length) {
              console.error(
                "[AIService] initial tool-call exposure violation",
                {
                  unexposed: exposure.unexposedNames,
                  requestTools: (tools || [])
        ...
        if (
          ...
          try {
            const proseBad =
              isNoToolAvailableProse(finalOutputText) ||
              isManualApiFallbackProse(finalOutputText);
            ...
            for (const n of requiredNames) {
              ...
              seenRequired.add(n);
              const hit = (Array.isArray(tools) ? tools : []).find(
                (s) => s?.function?.name === n,
              );
            ...
            if (plan) {
              console.log("[AIService] mcpEnforcement.recovery", {
                tools: plan.retryTools
                  .map((s) => s?.function?.name)
                  .filter(Boolean),
                forcedTool: plan.forcedTool || null,
                reason: proseBad
                  ? "false-unavailable-prose"
                  : emptyAnswer
                    ? "empty-answer"
                    : "ordinary-prose-skip",
              });
              ...
              if (outcome.recovered && outcome.toolCalls.length) {
                ...
                for (const s of outcome.tools) {
                  ...
                  if (
                    typeof n === "string" &&
                    n &&
                    !mcpProtectedNames.includes(n)
                  )
                    mcpProtectedNames.push(n);
                }
                ...
                if (outcome.response) {
                  assistantResponseMeta.provider =
                    outcome.response.provider || assistantResponseMeta.provider;
                  assistantResponseMeta.model =
                    outcome.response.model || assistantResponseMeta.model;
                  assistantResponseMeta.tokens =
                    outcome.response.tokens || assistantResponseMeta.tokens;
                }
              } else if (
                proseBad &&
                typeof outcome.text === "string" &&
                outcome.text &&
                !isNoToolAvailableProse(outcome.text) &&
                !isManualApiFallbackProse(outcome.text)
              ) {
                ...
                try {
                  const mismatchText =
                    "I couldn\u2019t complete that request with the tools available to me \u2014 none of them matched what was asked for, so nothing was created or changed. Please try again or rephrase the request.";
                  console.error("[AIService] mcpRecovery.capabilityMismatch", {
                    tools: (Array.isArray(outcome.unresolved)
                      ? outcome.unresolved
                      : []
                    )
                      .map((u) => u?.tool)
                ...
                proseBad &&
                Array.isArray(outcome.unresolved) &&
                outcome.unresolved.length &&
                outcome.unresolved.some(
                  (u) => Array.isArray(u.missing) && u.missing.length,
                )
              ) {
                ...
                try {
                  const first = outcome.unresolved.find(
                    (u) => Array.isArray(u.missing) && u.missing.length,
                  );
                  const schema =
                    this.resolvePendingSchema(first.tool, tools, mcpSchemas) ||
                    (mcpSchemas || []).find(
                      (s) => s?.function?.name === first.tool,
                    ) ||
                    null;
                  if (
                    schema &&
                    this.isPendingPermitted(first.tool, mcpSchemas)
                  ) {
                    const clarifyUpdateMode = this.upsertUpdateMode(schema, {
                      intentCaps: toolIntentCaps,
                      text: baseMessageContent,
                    });
                    const pend = createPending(
                      first.tool,
                      {},
                      schema,
                      Date.now(),
                      { updateMode: clarifyUpdateMode },
                    );
                    await this.savePendingToolCall(
                      conversationId,
                      userId,
                    ...
                    const effectiveParams = effectiveRequiredParams(schema, {
                      updateMode: clarifyUpdateMode,
                    });
                    const params = effectiveParams.filter((p) =>
                      first.missing.includes(p.name),
                    );
                    let question = buildClarification(
                      first.tool,
                      params.length ? params : effectiveParams.slice(0, 2),
                      {},
                    );
                    const optBits = [];
                    for (const u of outcome.unresolved) {
                      for (const [pname, items] of Object.entries(
                        u.options || {},
                      )) {
                        const list = (Array.isArray(items) ? items : [])
                          .filter(Boolean)
                          .slice(0, 5);
                        if (list.length)
                          optBits.push(`Possible ${pname}: ${list.join(", ")}`);
                      }
                    }
                  ...
                  /* clarification fallback must never throw */
                }
              }
            }
          } catch {
          ...
          try {
            ...
            if (
              ...
              console.error(
                "[AIService] ASSERT-5 false-unavailable prose with required MCP capability",
                {
      ...
      const makeContinuationGeneration = async ({
        assistantMessage,
        toolResults,
      }) => {
        ...
        const activeContinuationNames = activeToolNamesFromCalls(
        ...
        try {
          ...
          remainingRequired = (
            Array.isArray(recoveryRequiredNames) ? recoveryRequiredNames : []
          ).filter((n) => !done.has(n));
        } catch {
          remainingRequired = [];
        }
        ...
        console.log(
          "[AIService] provider continuation payload",
          ...
          {
            continuationTools: continuationPick.tools
              .map((t) => t?.function?.name)
              .filter(Boolean),
        ...
        try {
          continuationPrompt = this.refreshInventoryPrompt(
            systemPrompt,
            this.mcpInventoryBlockForTools(continuationPick.tools, {
              metadata: mcpMetadata,
              failures: [],
            }),
          );
        ...
        try {
          ...
          if (exposure.unexposedNames.length > 0) {
            console.error(
              "[AIService] continuation tool-call exposure violation",
              {
                unexposed: exposure.unexposedNames,
                continuationTools: continuationPick.tools
                  .map((t) => t?.function?.name)
      ...
      const runFollowUpContinuation = async (followUpCalls, siteOnChunk) => {
        const bounded = (
          Array.isArray(followUpCalls) ? followUpCalls : []
        ).slice(0, 4);
        const toolResults = [];
        for (const tc of bounded) {
          ...
          let result = null;
          try {
            result = await TaskExecutor.executeTool(
              fname,
              args,
              userId,
              socket,
        ...
        const gen2 = await makeContinuationGeneration({
        ...
        console.log(
          "[AIService] follow-up still tool-calling; flattened text synthesis",
          {
            followUpTools: followUp2
              .map((t) => t?.function?.name)
              .filter(Boolean),
          },
        );
        const flatContext = toolResults
          .map((r) => {
            ...
            try {
              ...
              body = "[unserializable tool result]";
            }
            if (body.length > 6000)
      ...
      if (toolCalls && toolCalls.length > 0) {
        ...
        if (toolCalls.length === 1 && !syntheticResponse) {
          ...
          const argSchema =
            this.resolvePendingSchema(execName, tools, mcpSchemas) ||
            this.resolvePendingSchema(rawName, tools, mcpSchemas);
          const permitted =
            this.isPendingPermitted(execName, mcpSchemas) ||
          ...
          if (argSchema && permitted && requiredParams(argSchema).length > 0) {
            ...
            try {
              resolveIntentCaps =
                classifyIntentCapabilities(baseMessageContent);
            } catch {
              resolveIntentCaps = null;
            }
            let parsedForMode = {};
            try {
              parsedForMode =
            ...
            const gateUpdateMode = this.upsertUpdateMode(argSchema, {
            ...
            const checked = (() => {
              try {
                const v = validateArgs(
                  argSchema,
                  applyParamDefaults(argSchema, parsed),
                );
                const extra = missingEffective(
                  argSchema,
                  (v && v.coerced) || parsed,
                  { updateMode: gateUpdateMode },
                );
                if (!extra.length) return v;
                const seen = new Set((v.errors || []).map((e) => e.name));
                return {
            ...
            if (!checked.ok) {
              const params = effectiveRequiredParams(argSchema, {
                updateMode: gateUpdateMode,
              });
              ...
              const pureCreate =
                resolveIntentCaps &&
                resolveIntentCaps.has("CREATE") &&
                !resolveIntentCaps.has("UPDATE") &&
                !resolveIntentCaps.has("DELETE");
              ...
              if (pureCreate) {
                ...
                if (
                  created &&
                  Array.isArray(created.toolCalls) &&
                  created.toolCalls.length > 0
                ) {
                  resolved = created;
                  ...
                  console.log("[AIService] creationContext.ask", {
                    tool: execName,
                    missing: missed,
                  });
                  ...
                  if (created.optionsNote)
                    question += ` ${created.optionsNote}`;
                  if (socket && !socket.isInterrupted) {
                    await this.emitAssistantText(socket, question);
                  }
                  return question;
              ...
              if (
                resolved &&
                Array.isArray(resolved.toolCalls) &&
                resolved.toolCalls.length > 0
              ) {
        ...
        const usePlanner =
          shouldUsePlanner(toolCalls, finalOutputText) &&
          preflightExecutedById.size === 0;
        if (usePlanner) {
          // Build plan steps from toolCalls
          const steps = toolCalls.map((toolCall) => {
            const fname = this.mapCalendarToolName(
          ...
          const exec = await TaskPlanner.createPlan({
            userId,
            workspaceId: workspaceContext.workspaceId,
            title: planTitle,
            prompt: text,
          ...
          const execResult = await TaskPlanner.executePlan(exec._id, socket, {
            controller,
            workspaceId: workspaceContext.workspaceId,
            conversationId,
          });
          ...
          try {
            await this.refreshWorkingStateFromResults(
              conversationId,
              userId,
              plannerToolResults,
            );
          } catch {
            /* working state is advisory */
          }
          ...
          if (
            ...
            if (assistantDraftMessageId && conversationId) {
              console.log(
                "[AIService] finalizing interrupted assistant draft",
                {
                  conversationId: String(conversationId),
                  messageId: String(assistantDraftMessageId),
              ...
              await Message.findByIdAndUpdate(assistantDraftMessageId, {
                ...
                model: assistantResponseMeta.model,
                metadata: {
                  tokens: assistantResponseMeta.tokens || {
                    input: 0,
                    output: 0,
                  },
                  streaming: false,
            ...
            if (execResult.status === "FAILED" && socket) {
              socket.emit("ai:agent:status", {
                status: "synthesizing",
                detail:
                  "Synthesizing a partial answer from recovered and failed steps.",
              });
            }
            ...
            finalOutputText = await consumeContinuation(
              finalGeneration,
              followUp,
              plannerStreamOnChunk,
            );
            if (!String(finalOutputText || "").trim() && followUp.length > 0) {
              console.log(
                "[AIService] continuation requested follow-up tools",
                {
                  tools: followUp.map((t) => t?.function?.name).filter(Boolean),
                },
              );
              finalOutputText = await runFollowUpContinuation(
                followUp,
                plannerStreamOnChunk,
              );
            }
          ...
          console.log(
            "[Planner] Delegating execution inline (quick tool path)",
          );
          const plannedTools = toolCalls.map((toolCall) =>
            this.mapCalendarToolName(toolCall.function.name, calendarIntent),
          );
          console.log(
            `[Planner] Chosen tools (in order): ${plannedTools.join(" -> ")}`,
          );
          ...
          try {
            const inlineMcp = plannedTools.filter(
              (n) => typeof n === "string" && n.startsWith("mcp_"),
            );
          ...
          for (const toolCall of toolCalls) {
            ...
            if (preExecuted) {
              try {
                if (
                  preExecuted.content &&
                  preExecuted.content.clientAction &&
                  socket
                ) {
                  socket.emit(
                    "ai:client:action",
              ...
              messages.push({
                ...
                content:
                  typeof preExecuted.content === "string"
                    ? preExecuted.content
            ...
            const functionName = this.mapCalendarToolName(
              toolCall.function.name,
              calendarIntent,
            );
            ...
            const toolResult = await TaskExecutor.executeTool(
              functionName,
              args,
              userId,
              socket,
            ...
            if (
              ...
              console.log(
                "[Planner] scheduleMeeting succeeded; running optional checkCalendar confirmation.",
              );
              ...
              const confirmationResult = await TaskExecutor.executeTool(
                "checkCalendar",
                confirmationArgs,
                userId,
                socket,
          ...
          const providerToolResults = toolCalls
            .map((toolCall) => {
          ...
          if (
            !String(finalOutputText || "").trim() &&
            quickFollowUp.length > 0
          ) {
        ...
        if (!initialStreamed && !conversationalStreamed) {
          ...
          if (socket) {
            await this.streamingRuntime.emitText(
              ...
              async (chunkText) => {
                assistantDraftContent += chunkText;
                // Speech is fed from the same delta as the visible draft.
                // Persist only after it is scheduled; the callback is run in
                // StreamingRuntime's background chain, but this ordering also
                // protects buffered/tool continuation responses.
                if (ttsBuffer) ttsBuffer.push(chunkText);
                if (voiceStreamer) voiceStreamer.push(chunkText);
                await persistAssistantDraft(assistantDraftContent, {
                  interrupted: false,
      ...
      // Voice failures never break text chat: flush errors are logged,
      // and persistence below runs regardless.
      if (ttsBuffer) {
        try {
      ...
      if (voiceStreamer) {
        try {
          ...
          console.warn(
            "[VoiceTTS] streamer flush failed:",
            error?.message || error,
          );
        }
      }
        ...
        if (conversationId) {
          try {
                  ...
                  try {
                    const rawArgs =
                      typeof tc?.function?.arguments === "string"
                        ? JSON.parse(tc.function.arguments)
                        : tc?.function?.arguments || {};
                    const vals = Object.values(rawArgs)
                      .map((v) =>
                        String(v ?? "")
                          .replace(/\s+/g, " ")
                          .trim(),
            ...
            console.log("[AIService] delivery.completed", {
              provider: assistantResponseMeta.provider || null,
              elapsedMs:
                deliveryTiming.deliveryDoneAt != null &&
                deliveryTiming.providerStartAt != null
                  ? deliveryTiming.deliveryDoneAt -
                    deliveryTiming.providerStartAt
                  : null,
            });
            console.log("[AIService] persistence.completed", {
              elapsedMs:
                deliveryTiming.persistenceDoneAt -
                deliveryTiming.providerStartAt,
            });
            ...
            console.error(
              "[AIService] Error saving conversation message:",
              err,
            );
          }
        }
      ...
      if (conversationId && !isGuest && socket && socket.isInterrupted) {
        const interruptedTitleSeed = String(
          assistantDraftContent || finalOutputText || "",
        ).trim();
      ...
      if (isAbortError) {
        ...
        // AbortSignal propagation already cancels in-flight synthesis;
        // these stop() calls emit the terminal voice events.
        if (ttsBuffer) ttsBuffer.stop();
        if (voiceStreamer) voiceStreamer.stop("interrupted");
        ...
        if (assistantDraftMessageId && conversationId) {
          ...
          try {
            ...
            console.warn(
              "[AIService] Failed to persist interrupted draft:",
      ...
      console.error(
        "[AIService] provider failure summary",
        describeProviderFailure(error, {
          providerId:
            assistantResponseMeta.provider || error?.providerId || null,
          ...
          tools:
            typeof tools !== "undefined" && Array.isArray(tools)
              ? tools.length
              : null,
          hasAttachments: Boolean(
            imageBase64 || (document && (document.data || document.url)),
          ),
          stream: false,
        }),
      );
