...
const Modal = styled.div`
  background:var(--surface); padding:18px; border-radius:12px; width:420px; color:var(--foreground); border:1px solid rgba(100,120,180,0.12);
`;
          ...
          {mode === 'pairing' ? (
            pairingCode ? (
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:12,color:'var(--foreground-muted)',marginBottom:6}}>Pairing code</div>
                <div style={{fontSize:28,fontWeight:800,letterSpacing:'0.16em',color:'var(--accent-soft)'}}>{pairingCode}</div>
              </div>
            ) : (
              <div style={{color:'var(--foreground-muted)'}}>Waiting for pairing code...</div>
            )
          ) : qr ? <img src={qr} alt="WhatsApp QR" style={{maxWidth:'100%',maxHeight:160}} /> : <div style={{color:'var(--foreground-muted)'}}>Waiting for QR...</div>}
        </QRBox>

        <Row>
          <Info>
            Status: <strong style={{color: status === 'ready' ? 'var(--accent-soft)' : 'var(--warning)'}}>{status}</strong>
            <br />
            {mode === 'pairing'
